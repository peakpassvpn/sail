...
fn outbound(json: serde_json::Value) -> sail::config::Outbound {
    serde_json::from_value(json).unwrap()
}

fn inbound(json: serde_json::Value) -> sail::config::Inbound {
    serde_json::from_value(json).unwrap()
}

/// Whether a connection through the socks inbound on `port` reaches the
/// echo server.
async fn relayed(port: u16, sess: &sail::session::Session) -> bool {
