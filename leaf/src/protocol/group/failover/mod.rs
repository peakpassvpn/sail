...
async fn single_health_check(
    ...
    match network {
        Network::Tcp => {
            let stream =
                match crate::net::connect_stream_outbound(&sess, dns_client.clone(), &h).await {
                    Ok(s) => s,
            ...
            match h.handle(&sess, None, stream).await {
                Ok(stream) => {
                    let Ok(tls_handler) = crate::transport::tls::outbound::StreamHandler::new(
                        String::from(""),
                        vec![],
                        None,
                        false,
        ...
        Network::Udp => {
            let transport = match crate::net::connect_datagram_outbound(&sess, dns_client, &h).await
            {
                Ok(t) => t,
                Err(_) => return Measure::new(idx, u128::MAX, tag),
            };
