...
async fn relayed(sess: &leaf::session::Session) -> anyhow::Result<bool> {
...
fn a_failed_reload_changes_nothing() -> anyhow::Result<()> {
    ...
    let dir = std::env::temp_dir().join(format!("leaf-test-reload-{}", std::process::id()));
    ...
    let opts = leaf::StartOptions {
        config: leaf::Config::File(path.to_string_lossy().to_string()),
    ...
    rt.spawn_blocking(move || leaf::start(id, opts));
    ...
    let result = rt.block_on(async {
        ...
        let reload = tokio::task::spawn_blocking(move || leaf::reload(id)).await?;
